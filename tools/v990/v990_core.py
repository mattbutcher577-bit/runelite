...
def validate_snapshot(s, max_age_ms=2500, now_ms=None):
    ...
    now_ms=_i(now_ms, int(time.time()*1000)); generated=_i(s.get('generatedAtEpochMs'),0)
    ...
    if not (s.get('safety') or {}).get('bridgeReady'): raise V990Error('BRIDGE_STALE','safety.bridgeReady false')
...
def valid_bounds(b): return isinstance(b,dict) and bool(b.get('valid')) and _i(b.get('width'))>0 and _i(b.get('height'))>0
def require_bounds(b, code='ACTION_BOUNDS_INVALID'):
    if not valid_bounds(b): raise V990Error(code,'invalid RuneLite bounds')
    return b

def contains(b,x,y): return valid_bounds(b) and _i(b['x']) <= _i(x) < _i(b['x'])+_i(b['width']) and _i(b['y']) <= _i(y) < _i(b['y'])+_i(b['height'])
def safe_target_point(b, inset_ratio=.18, min_inset=2):
    b=require_bounds(b,'MOUSE_TARGET_BOUNDS_INVALID'); x,y,w,h=map(_i,(b['x'],b['y'],b['width'],b['height']))
    ix=min(max(min_inset,int(w*inset_ratio)), max(0,(w-2)//2)); iy=min(max(min_inset,int(h*inset_ratio)), max(0,(h-2)//2))
...
def canvas_to_screen(s, point):
    validate_snapshot(s); c=s['client']; x,y=map(_i,point); cw,ch=_i(c['canvasWidth']),_i(c['canvasHeight'])
...
def find_ge_inventory_entry(s,item_id,minimum_quantity=1):
    ...
    matches=[e for e in gi.get('entries') or [] if (_i(e.get('canonicalItemId'),-1)==_i(item_id) or _i(e.get('rawItemId'),-1)==_i(item_id)) and _i(e.get('quantity'),0)>=_i(minimum_quantity,1)]
    if not matches: raise V990Error('SELL_ITEM_NOT_FOUND',f'itemId={item_id} qty>={minimum_quantity}')
    matches.sort(key=lambda e:(-_i(e.get('quantity')), _i(e.get('inventorySlot'),999))); return matches[0]
def verify_mouse_event(s,event_type,button,bounds,after_seq):
    ...
    for e in (s.get('mouse') or {}).get('recentEvents') or []:
        if _i(e.get('eventSeq'),0)>_i(after_seq) and str(e.get('type','')).upper()==str(event_type).upper() and _i(e.get('button'),0)==_i(button) and contains(bounds,e.get('canvasX'),e.get('canvasY')): return True
    return False
def verify_offer_confirmation(s,slot_one_based,item_id,qty,price,allowed_states):
    row=find_slot(s,slot_one_based); st=str(row.get('state','')).upper()
...
def shadow_rank(features):
    profit=max(0.0,_f(features.get('profit_after_tax'))); p=min(1.0,max(0.0,_f(features.get('fill_probability'),.5))); fill=max(1.0,_f(features.get('expected_fill_seconds'),60.0)); downside=max(0.0,_f(features.get('downside'))); capital=max(1.0,_f(features.get('capital'),1.0)); volume=max(0.0,_f(features.get('volume')))
    expected=profit*p-downside*(1.0-p); score=expected/(1.0+fill/120.0)*(0.75+min(.5,math.log1p(volume)/10.0))+(expected/capital)*100.0
